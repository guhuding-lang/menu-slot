...
function joinPage(error = "") {
  return `<main class="join-page"><h1 class="join-mark">嘎巴47</h1><section class="join-copy"><h1>一起练<br />别一起躺</h1><p>属于47群的私人健身打卡局。</p></section><form class="join-form" id="join-form"><label><span>群邀请码</span><input name="code" placeholder="请输入群邀请码" value="" autocomplete="off" autocapitalize="characters" /></label><label><span>你的昵称</span><input name="name" placeholder="群友认得出的名字" maxlength="20" autocomplete="nickname" /></label>${error ? `<p class="form-error">${escapeHTML(error)}</p>` : ""}<button class="primary-button" type="submit">加入健身局 ${icon("arrow-right")}</button><p class="privacy-note">${icon("lock-simple")} 打卡只对47群成员可见</p></form></main>`;
}
